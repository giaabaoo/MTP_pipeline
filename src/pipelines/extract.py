...
class Extract():
    ...
    def run(self):
        ...
        for conversation_name, conversation_data in data_loader.get_items().items():
            ...
            if self.config.scene_describer.name == "LLAVA":
                '''
                SCENE DESCRIBER
                '''
                print("===================== SCENE DESCRIBING =====================")
                # utterance_level_video_descriptions = scene_describer.main_run(conversation_data, utterance_level_videos)
                utterance_level_video_descriptions = ""
                # '''
                # REASONER
                # '''
                print("===================== REASONING WITH CHATGPT =====================")   
                print("===================== SUMMARIZING =====================")
                video_summary = reasoner.main_run_summarizer_transcripts_only(conversation_data, utterance_level_videos, utterance_level_transcripts)
            
            elif self.config.scene_describer.name == "GPT4V":
                '''
                SCENE DESCRIBER + SUMMARIZE
                '''
                print("===================== SCENE DESCRIBING + SUMMARIZING =====================")

                utterance_level_video_descriptions, video_summary = scene_describer.main_run(conversation_data, utterance_level_videos, utterance_level_transcripts)
